...
function shell(){el('app').innerHTML=`<div class="shell"><aside class="side"><div class="brand"><b>RISKSHIELD AI</b><span>MERCHANT RISK MANAGER</span></div><div class="status">● SYSTEM ONLINE <span class="pill">RAZORPAY LIVE</span></div><div class="navlabel">OPERATIONS</div><div class="nav">${nav.slice(0,7).map(n=>`<button data-tab="${n[0]}">${n[1]} ${n[0]=='alerts'?'<span id="alertCount"></span>':''}</button>`).join('')}<div class="navlabel">SYSTEM</div>${nav.slice(7).map(n=>`<button data-tab="${n[0]}">${n[1]}</button>`).join('')}</div><div class="footer">Synthetic data only • v1.0.0<br>Defense-only risk scoring</div></aside><main class="main"><div class="top"><div class="title"><h1 id="pageTitle">Risk Dashboard</h1><p>Real-time transaction risk intelligence</p></div><div class="actions"><button class="btn" id="testBtn">Send Test Event</button><button class="btn primary" id="runBtn">Connect Live Feed</button></div></div>${nav.map(n=>`<section id="tab-${n[0]}" class="tab"></section>`).join('')}</main></div><div class="modal" id="modal"><div class="modalbox" id="modalbox"></div></div>`;document.querySelectorAll('.nav button').forEach(b=>b.onclick=()=>setTab(b.dataset.tab));el('runBtn').onclick=toggleRun;el('testBtn').onclick=sendTestEvent;setTab('overview');loadAll();connect()}
...
function renderSettings(){el('tab-settings').innerHTML=`<div class="card"><h2>LIVE INGESTION</h2><div class="factor">Source: Razorpay Webhooks</div><div class="factor">Events: payment.authorized, payment.captured, payment.failed</div><div class="factor">WebSocket: /ws/transactions</div><div class="factor">Signature: X-Razorpay-Signature (HMAC-SHA256)</div><div class="factor">Database: Server-side SQLite (PostgreSQL-ready)</div><div class="factor">Demo endpoint: /api/v1/demo/event</div><div class="small">The dashboard is server-driven. Test Event uses the same scoring, database and WebSocket pipeline as an incoming payment event. Razorpay can later POST to /api/v1/webhooks/razorpay.</div><button class="btn" onclick="resetDemo()">Clear Demo Events</button></div>`}
async function resetDemo(){await api('/api/v1/simulator/reset',{method:'POST'});state.tx=[];loadAll()}
async function showTx(id){const t=await api('/api/v1/transactions/'+id);el('modal').classList.add('open');el('modalbox').innerHTML=`<div style="display:flex;justify-content:space-between"><h2>${t.id}</h2><button class="btn" onclick="el('modal').classList.remove('open')">Close</button></div><div class="grid"><div class="factor">Risk<br><b class="${t.risk_level.toLowerCase()}">${t.risk_level} ${t.risk_score}</b></div><div class="factor">Decision<br><b>${t.decision}</b></div><div class="factor">Amount<br><b>₹${t.amount.toFixed(2)}</b></div><div class="factor">Customer<br><b>${t.customer_id}</b></div></div><h2>WHY THIS SCORE</h2>${t.factors.map(f=>`<div class="factor">${f.factor}<span style="float:right">+${f.weight}</span></div>`).join('')||'<div class="small">No dominant risk factors.</div>'}<div class="toolbar" style="margin-top:15px"><button class="btn" onclick="workflow('${t.id}','VERIFY')">Verify</button><button class="btn" onclick="workflow('${t.id}','ALLOW')">Allow</button><button class="btn" onclick="workflow('${t.id}','RESOLVE')">Resolve</button></div>`}
